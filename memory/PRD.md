...
- DONE (2026-08-04): AMPLIACIÓN DE VÍDEOS. VIDEO_IDS ampliado a **41/46 ejercicios con miniatura de vídeo curada** (YouTube IDs validados por oEmbed). Los **5 restantes** (Encogimientos, Extensión de Tríceps en Polea, Fondos, Peck Deck, Pullover) usan fallback de búsqueda "Ver en YouTube". Verificado visualmente (screenshot): Press Militar (Hombros) muestra miniatura con play; Fondos (Brazos) muestra botón de búsqueda fallback. Frontend compila (solo warnings eslint). Proveniencia: agent-tested por screenshot, no confirmado por usuario. NO reintentar iframe embed (bloqueado en este entorno).
